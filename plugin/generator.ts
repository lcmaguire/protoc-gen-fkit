...
const genMessage = "<!-- this file was generated by <todo put plugin name> do not edit-->"
...
export function basicEdit(schema: Schema, message: DescMessage) {
    ...
    for (let i in message.fields) {
        ...
        if (currentField.repeated) {
            ...
            end = `${spacing}<button on:click={() => remove${currentFieldName}Array(key)}> Remove </button>\n`

            end += `  {/each}\n`
            end += `{/if}\n`
            end += `<button on:click={push${currentFieldName}Array}> Add </button>\n`
